...
class Trainer:
    ...
    def postprocess(self):
        ...
        """
        self.outputs["results"][0] = self.arch.postprocess(self.outputs["results"])
    ...
    def test(cls, cfg):
        ...
        with torch.no_grad():
            for batch_data in tqdm(test_dataloader,
                ...
                if input_frames.shape[1] == 4:
                   input_frames = input_frames
                else:
                    A,B,C = torch.split(input_frames,1,1)
                    input_frames = torch.cat((A,B,C,outputs2), dim=1)
                # record the testing time.
                torch.cuda.synchronize()
                time_start = time.time()
                if hasattr(arch, "inference"):
                    outputs = arch.postprocess(arch.inference(input_frames))[0]
                else:
                    outputs = arch.postprocess(arch.model(input_frames))[0]
                torch.cuda.synchronize()
                ...
                outputs2 = outputs[0].unsqueeze(0)
