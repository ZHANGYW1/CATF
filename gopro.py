...
class GOPRO(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_files = [f for f in os.listdir(os.path.join(self.cfg.root_gt, video_name, "blur_gamma")) if f.endswith('.png')]
        sorted_images = sorted(image_files)

        input_frames = np.stack(input_frames, axis=0).transpose([0, 3, 1, 2])
        last_output = np.stack(last_output, axis=0).transpose([0, 3, 1, 2])

        if frame_name in [sorted_images[0],sorted_images[1],sorted_images[2],sorted_images[3],sorted_images[4]]:
