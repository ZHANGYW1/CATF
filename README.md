...
1. Download SimDeblur
 ```bash
git clone https://github.com/ljzycmd/SimDeblur.git
```
2. Replace the path SimDeblur\simdeblur\dataset\dvd.py with [`dvd.py`](./dvd.py)  
In last_output_path = os.path.join("the path", video_name, "GT", "{}"), please replace "the path" with the input path of the first frame.  

3. Replace the path SimDeblur\simdeblur\dataset\gopro.py with [`gopro.py`](./gopro.py)  
...
The repo is partly built based on [VDTR](https://github.com/ljzycmd/VDTR), [NAFNet](https://github.com/megvii-research/NAFNet), and MoE. We are grateful for their generous contribution to open source.
