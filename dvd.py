...
class DVD(torch.utils.data.Dataset):
        ...
        for video_name in self.video_list:
            ...
            if cfg.sampling == "n_n" or cfg.sampling == "n_l":
                ...
                self.frames += [frames_in_video[int(idx)]
                                for idx in np.linspace(2, len(frames_in_video)-3, len(frames_in_video)-2)]
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.sampling == "n_c":
            ...
            if input_frames_name[0] == '-0001.jpg':
               input_frames_name[0] = "00000.jpg"
        ...
        last_output_path = os.path.join("the path", video_name,"GT","{}")
        ...
        for frame_name in gt_frames_name:
            ...
            last_frame = f"{frame_number - 1:05d}.jpg"
            if os.path.exists(last_output_path.format(last_frame)):
                last_frame = last_frame
            else:
                last_frame = frame_name
            last_frames.append(last_frame)

        last_output = [read_img_opencv(last_output_path.format(
            last_frame)) for last_frame in last_frames]

        input_frames = [read_img_opencv(input_frames_path.format(
            frame_name)) for frame_name in input_frames_name[2:]]

        gt_frames = np.stack(
            gt_frames, axis=0)[..., ::-1].transpose([0, 3, 1, 2])
        input_frames = np.stack(
            input_frames, axis=0)[..., ::-1].transpose([0, 3, 1, 2])
        last_output = np.stack(
            last_output , axis=0)[..., ::-1].transpose([0, 3, 1, 2])


        if frame_name in ["00002.jpg","00001.jpg","00000.jpg"]:
            input_frames = np.concatenate([input_frames, last_output], axis=0)
        else:
